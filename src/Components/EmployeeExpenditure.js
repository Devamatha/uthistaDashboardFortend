...
function EmployeeExpenditure() {
  ...
  const fetchData = async (page, size) => {
    try {
      ...
      axios
        .get(`${apiUrl}salaries/getAllEmployee`, {
          params: {
            page,
            size,
          },
  ...
  useEffect(() => {
    const filtered = data?.filter((item) => {
      ...
      const matchesFormType =
        !selectedFormType || item.formType === selectedFormType;

      const matchescategoryType =
        !selectedCategoryType || item.categoryType === selectedCategoryType;
    ...
    <div className="rounded-sm max-w-1440 whitespace-nowrap  mt-3 bg-white w-full ">
      ...
      <div>
        <div className="mb-4 flex justify-start align-items-center  shrink-0">
          ...
          <div>
            ...
            >
              ...
              <option value="stafftrainingexpenses">
                {" "}
              ...
              <option value="maintenanceandrepair">
                {" "}
        ...
        {isLoading ? (
          ...
          <div className="table-container  table-responsive text-center rounded shadow-sm rounded-sm ">
            ...
            <table className="table-active table-striped table-hover  rounded-md overflow-x-auto">
              <thead className="font-work-sans text-[20px] font-medium not-italic  leading-normal bg-[#FF914D]  ">
                <tr className="rounded-md">
                  ...
                  <th className="py-[16px] px-[24px]">formType</th>
                  <th className="py-[16px] px-[24px]">CategoryType</th>
              ...
              <tbody className="font-work-sans text-[18px] font-medium not-italic leading-normal">
                {currentItems?.map((data, index) => (
                  <tr key={index} className="border">
                    ...
                    <td className="py-[16px] px-[24px]">{data.empId}</td>
                    <td className="py-[16px] px-[24px]">{data.empName}</td>

                    <td className="py-[16px] px-[24px]">{data.formType}</td>
                    <td className="py-[16px] px-[24px]">{data.categoryType}</td>
                    <td className="py-[16px] px-[24px]">{data.salaryAmount}</td>
                    <td className="py-[16px] px-[24px]">{data.paymentMode}</td>
                    <td className="py-[16px] px-[24px]">
                      {data.transactionId}
                    </td>
                    <td className="py-[16px] px-[24px]">
